...
import 'package:bazz_flutter/modules/home_module/views/map/cluster_manager/google_maps_cluster_manager.dart';
...
class ClusterManager<T> {
  ClusterManager(this._items, this.updateMarkers,
      ...
      required this.stopClusteringZoom})
  ...
  Future<Set<Marker>> updateMap() async {
    final Set<Marker> markers = await _updateClusters();
    return markers;
  }

  Future<Set<Marker>> _updateClusters() async {
  ...
  Future<Set<Marker>> setItems(List<ClusterItem<T>> newItems,
      {bool updateMarkers: true}) async {
    _items = newItems;
    final Set<Marker> markers = await updateMap();
    return markers;
  }
  ...
  void updateZoomLevel(double zoom, {bool forceUpdate = false}) {
    _zoom = zoom;
    if (forceUpdate) {
      updateMap();
    }
