...
class _QuickHullDistantPoint {
  ...
  _QuickHullDistantPoint({required this.maxPoint, required this.newPoints});
}
...
class QuickHull {
  ...
  static _QuickHullDistantPoint _findMostDistantPointFromBaseLine(
    ...
    for (var i = latLngs.length - 1; i >= 0 != null; i--) {
      var pt = latLngs[i];
      var d = _getDistant(pt as LatLng, baseLine as List<LatLng>);

      if (d > 0) {
        newPoints.add(pt as LatLng);
      } else {
        continue;
      }
