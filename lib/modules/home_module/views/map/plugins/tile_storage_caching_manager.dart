...
class TileStorageCachingManager {
  ...
  static final HashMap<num, dynamic> _tilesMemoryCache = HashMap();
  late Database _db;
  ...
  static Future<Tuple2<Uint8List, DateTime>> getTile(Coords coords,
    ...
    TelloLogger().i(
        "======> getTile ${coords.toString()} ${_tilesMemoryCache.keys.length}");
    final key = coords.x + coords.y + coords.z;
    if (_tilesMemoryCache.containsKey(key)) {
      TelloLogger().i("======> getTile from cache ${coords.toString()}");
      return _tilesMemoryCache[key] as Tuple2<Uint8List, DateTime>;
    }
    final query = 'select $_kTileDataColumn, $_kUpdateDateColumn from tiles '
        'where $_kZoomLevelColumn = ${coords.z} AND '
        '$_kTileColumnColumn = ${coords.x} AND '
        '$_kTileRowColumn = ${coords.y} limit 1';

    final List<Map> result =
        await (await _getInstance().database).rawQuery(query);

    final tupleResult = result.isNotEmpty
        ? Tuple2(
            result.first[_kTileDataColumn] as Uint8List,
            DateTime.fromMillisecondsSinceEpoch(
                1000 * (result.first[_kUpdateDateColumn] as int)))
        : null;

    _tilesMemoryCache[key] = tupleResult;
  ...
  static Future<void> saveTile(Uint8List tile, Coords cords) async {
    TelloLogger().i("======> saveTile ${cords.toString()}");
    final key = cords.x + cords.y + cords.z;
    final currentDateTime = DateTime.now().millisecondsSinceEpoch ~/ 1000;
    if (!_tilesMemoryCache.containsKey(key)) {
      _tilesMemoryCache[key] = Tuple2(tile, currentDateTime);
    }
    await (await _getInstance().database).insert(
        _kTilesTable,
        {
          ...
          _kUpdateDateColumn: currentDateTime,
  ...
  static Future<void> changeMaxTileAmount(int maxTileAmount) async {
    ...
    await db.transaction((txn) async {
      ...
      final currentTilesAmount = currentTilesAmountResult.isNotEmpty
          ? currentTilesAmountResult.first['count(*)'] as int
          : 0;
  ...
  static Future<int> get maxCachedTilesAmount async {
    ...
    return result.isNotEmpty
        ? int.parse(result.first[_kConfigValueColumn].toString())
