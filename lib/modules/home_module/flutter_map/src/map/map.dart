...
import 'package:bazz_flutter/modules/home_module/flutter_map/src/core/center_zoom.dart';
...
class MapControllerImpl implements MapController {
  Completer<MapState> _readyCompleter = Completer<MapState>();
  ...
  @override
  Future<MapState> get onReady => _readyCompleter.future;

  void dispose() {
    TelloLogger().i("MapControllerImpl Map DIspose");
    _readyCompleter = Completer<MapState>();
    //_mapEventSink.close();
  }
  ...
  set state(MapState state) {
    ...
    if (!_readyCompleter.isCompleted) {
      _readyCompleter.complete(state);
    }
  }

  @override
  CustomPoint project(LatLng latlng, [double? zoom]) {
    CustomPoint pos = _state.project(latlng, zoom!);
    // ignore: join_return_with_assignment
    pos = pos.multiplyBy(_state.getZoomScale(_state.zoom, _state.zoom)) -
  ...
  LatLng unproject(CustomPoint point, [double? zoom]) {
    return _state.unproject(point, zoom!);
  }
...
class MapState {
  ...
  late LatLngBounds _lastBounds;
  late Bounds _lastPixelBounds;
  ...
  late CustomPoint _originalSize;
  ...
  void dispose() {
    TelloLogger().i("Call Map DIspose");
  ...
  bool move(LatLng center, double zoom,
    ...
    TelloLogger().i("Move to  zoom 0000 ==> $zoom");
    if (_lastCenter != null && (!mapMoved || !bounds.isValid)) {
      return false;
    }
    TelloLogger().i("Move to  zoom 111111111 ==> $zoom");
    ...
    TelloLogger().i("Move to  zoom 22222222 ==> $zoom");
    _zoom = zoom;
    _lastCenter = center;
    ...
    if (options.onPositionChanged != null) {
      var mapPosition = MapPosition(
          ...
          bounds: bounds,
          zoom: zoom,
          hasGesture: hasGesture as bool);
      options.onPositionChanged!(mapPosition, hasGesture as bool);
    }
    TelloLogger().i("Move to  zoom 3333333 ==> $zoom");
    return true;
  }
  ...
  double getBoundsZoom(LatLngBounds bounds, CustomPoint<double> padding,
      {bool inside = false}) {
    var zoom = this.zoom;
    var min = options.minZoom;
    var max = options.maxZoom;
