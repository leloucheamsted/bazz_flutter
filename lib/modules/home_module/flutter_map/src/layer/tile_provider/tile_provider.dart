...
abstract class TileProvider {
  ...
  String getTileUrl(Coords coords, TileLayerOptions options) {
    ...
    final url = options.templateFunction(urlTemplate, allOpts);
    TelloLogger().i("options.templateFunction ===> $url");
    return url;
  }

  double _getZoomForUrl(Coords coords, TileLayerOptions options) {
    var zoom = coords.z;
    TelloLogger().i("_getZoomForUrl $zoom ,,, ${options.zoomOffset}");
    if (options.zoomReverse) {
      zoom = options.maxZoom - zoom;
    }

    return (zoom += options.zoomOffset) as double;
  }
