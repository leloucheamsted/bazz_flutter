...
import 'package:bazz_flutter/modules/home_module/flutter_map/src/core/bounds.dart';
import 'package:bazz_flutter/modules/home_module/flutter_map/src/core/point.dart';
import 'package:bazz_flutter/modules/home_module/flutter_map/src/core/util.dart'
    as util;
...
class TileLayerOptions extends LayerOptions {
  ...
  late WMSTileLayerOptions? wmsOptions =
      new WMSTileLayerOptions(baseUrl: 'baseUrl');
  ...
  /// maxNativeZoom level ad auto-scaled.
  ...
  TileLayerOptions({
    ...
    this.templateFunction = util.template,
        ...
        super(key: key as Key, rebuild: rebuild as Stream<Null>);
}
...
class _TileLayerState extends State<TileLayer> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _resetView();
    _update(null as LatLng);
    _moveSub = widget.stream!.listen((_) => _handleMove());

    _connectivitySub?.cancel();
    _connectivitySub = DataConnectionChecker().onStatusChange.listen((status) {
      if (status == DataConnectionStatus.disconnected) {
        ...
        TelloLogger()
            .i('[TileLayerState] You are disconnected from the internet.');
        _removeAllTiles(true);
        _resetView();
        _update(null as LatLng);
      }
    });
  ...
  void didUpdateWidget(TileLayer oldWidget) {
    ...
    TelloLogger().i("didUpdateWidget $reloadTiles");
    if (!reloadTiles) {
  ...
  void _abortLoading() {
    ...
    for (var key in toRemove) {
      ...
      tile!.tileReady = null as Function(Coords<double>, dynamic, Tile);
      tile.dispose();
      _tiles.remove(key);
    }
  }
  ...
  Level _updateLevels() {
    ...
    if (zoom == null) return null as Level;
      ...
      setState(() {
        if ((tileZoom - _tileZoom!).abs() >= 1) {
          ...
          if (null == _throttleUpdate) {
            ...
            _throttleUpdate!.add(null as LatLng);
          }
  ...
  void _removeTile(String key, [bool evict = false]) {
    var tile = _tiles[key];
    if (tile == null) {
      return;
    }
  ...
  void loadTileImage() {
    try {
      ...
      if (_imageStream!.key != oldImageStream?.key) {
        ...
        _listener = ImageStreamListener(_tileOnLoad,
            onError: _tileOnError as Function(Object, StackTrace?));
