...
import 'package:bazz_flutter/modules/home_module/flutter_map/src/gestures/latlng_tween.dart';
...
abstract class MapGestureMixin extends State<FlutterMap>
  ...
  int _tapUpCounter = 0;
  late Timer _doubleTapHoldMaxDelay;
  ...
  void _handleDoubleTapZoomAnimation() {
    mapState.move(
      _doubleTapCenterAnimation.value as LatLng,
      _doubleTapZoomAnimation.value as double,
      hasGesture: true,
      source: MapEventSource.doubleTapZoomAnimationController,
    );
  }
  ...
  void _handleDoubleTapHold(ScaleUpdateDetails details) {
    ...
    if (InteractiveFlag.hasFlag(flags, InteractiveFlag.pinchZoom)) {
      ...
      final min = options.minZoom;
      final max = options.maxZoom;
