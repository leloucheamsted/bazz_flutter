...
class DataConnectionChecker {
 ...
 late  List<AddressCheckOptions> addresses = DEFAULT_ADDRESSES;
  ...
  /// This is a singleton that can be accessed like a regular constructor
  /// i.e. DataConnectionChecker() always returns the same instance.
 late  factory DataConnectionChecker() => _instance;
  ...
  DataConnectionChecker._() {
    ...
    _statusController.onCancel = () {
      _timerHandle?.cancel();
      late  DataConnectionStatus ? lastStatus ; // reset last status
    };
  }
...
late  DisconnectionStatus ? _disconnectionStatus;

  DisconnectionStatus get disconnectionStatus => _disconnectionStatus!;

 late AddressCheckOptions2 ? _serviceAddress;
  ...
  Future<bool> get hasConnection async {
    ...
    bool result = _lastTryResults.map((result) => result.isSuccess).contains(true);

    if (!result) {
      _disconnectionStatus = DisconnectionStatus.noInternet;
    } else if (serviceAddress != null) {
      try {
        final response = await http.get("${serviceAddress.address}/${serviceAddress.method}" as Uri).timeout(DEFAULT_TIMEOUT) ;
  ...
  Future<bool> get isConnectedToInternet async {
    final ConnectivityStatus connectivityStatus = await Connectivity().checkConnectivity().timeout(const Duration(seconds: 10),onTimeout: (){
  ...
  _maybeEmitStatusUpdate([Timer ? timer]) async {
    ...
    _timerHandle?.cancel();
    timer?.cancel();
    DataConnectionStatus currentStatus;
  ...
  String toString() {
    String message = "";
    if (DataConnectionChecker().disconnectionStatus == DisconnectionStatus.noConnectivity) {
      message =
          "{LocalizationService().of().noInternetConnection} - LocalizationService().of().noWifiOrCellular}";
    } else if (DataConnectionChecker().disconnectionStatus == DisconnectionStatus.noInternet) {
      message =
          "{LocalizationService().of().noInternetConnection} - {LocalizationService().of().noInternetData}";
    } else if (DataConnectionChecker().disconnectionStatus == DisconnectionStatus.noService) {
      message =
          "{LocalizationService().of().noInternetConnection} - {LocalizationService().of().noSystemService}";
    }
    else{
      message = "LocalizationService().of().unknownMsg";
    }
    return message;
  }
}
